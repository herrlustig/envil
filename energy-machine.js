...
function handleMessage(msg) {
    switch (msg.type) {
        ...
        case 'play-pattern-now': {
            const pool = sanitizePoolName(msg.pool);
            const patId = String(msg.patId || 'manual');
            const text = String(msg.text || '');
            if (!pool || !text.trim()) return;
            const src = scStr(text);
            const id = scStr(patId);
            const code = `Library.at(\\envil, \\energyPlayNow) !? { |f| f.value(\\${pool}, ${src}, ${id}) }`;
            if (code.length > MAX_PAYLOAD) {
                log(`⚡ [energy] play-now skipped (pattern too long: ${pool}/${patId})`);
                return;
            }
            sendSC(code, true);
            return;
...
function buildEnergyBackboneCode() {
    return [
        ...
        `  out.asArray;`,
        `});`,
        `Library.put(\\envil, \\energyApplyOvr, { |pat, pairs, scope, node, patId|`,
        `  var out = pat;`,
        `  var lst = pairs ? [];`,
        `  if(lst.size <= 0, { ^out });`,
        `  lst.clump(2).do { |kv|`,
        `    var k = kv[0], v = kv[1];`,
        `    var cand = try { Pbindf.performList(\\new, [out, k, v]) } { |err|`,
        `      ("[energy] " ++ scope ++ " override skipped (" ++ node ++ "/" ++ patId ++ ") key=" ++ k ++ ": " ++ err.errorString).warn;`,
        `      nil`,
        `    };`,
        `    if(cand.notNil, {`,
        `      // Probe one event at runtime to catch bad value logic (e.g.`,
        `      // doesNotUnderstand inside Pfunc/Pkey chains).`,
        `      var ok = try { cand.asStream.next(Event.default); true } { |err|`,
        `        ("[energy] " ++ scope ++ " override ignored at runtime (" ++ node ++ "/" ++ patId ++ ") key=" ++ k ++ ": " ++ err.errorString).warn;`,
        `        false`,
        `      };`,
        `      if(ok, { out = cand });`,
        `    });`,
        ...
        `      var item, node, quant, mode, str, b, maxWait, waited;`,
        ...
        `        str = sp.par(Pfset({}, Pprotect(item[0], { |err|`,
        `          ("[energy] pattern error in " ++ name ++ ": " ++ err.errorString).warn;`,
        `          item[1].test = true; item[1].signal;`,
        `        }), { item[1].test = true; item[1].signal }));`,
        `        item[2] !? { Library.put(\\envil, \\energyPlaying, item[2], [name, str, item[1]]) };`,
        `        mode = (node !? { node[\\mode] }) ? \\seq;`,
        `        if(mode != \\par, {`,
        `          maxWait = (((Library.at(\\envil, \\energyCfg) ? ())[\\maxDur]) ? 20).max(0.1) + 2;`,
        `          waited = 0;`,
        `          while({ item[1].test.not and: { waited < maxWait } }, { sp.wait(0.1); waited = waited + 0.1 });`,
        `          if(item[1].test.not, {`,
        `            ("[energy] seq wait timeout in " ++ name ++ " (forcing release)").warn;`,
        `            item[1].test = true; item[1].signal;`,
        `          });`,
        `        });`,
        `      }, { sp.wait(0.1) });`,
...
function buildEnergyTokenCode() {
    return [
        ...
        `  rout = Routine({`,
        `    var pos = (startPos ? \\source).asSymbol, graph, cfg, addr, node, outs, dest, pats, pick, patObj, cond, w, dwell, travel, pickO, tdur, tf, prog, c2, dd, sl, maxWait, waited;`,
        ...
        `            patObj = try {`,
        `              patObj = Library.at(\\envil, \\energyApplyOvr).value(patObj, po, "pool", pos.asString, pick[\\id].asString);`,
        `              patObj = Library.at(\\envil, \\energyApplyOvr).value(patObj, go, "global", pos.asString, pick[\\id].asString);`,
        `              Pfindur(cfg[\\maxDur] ? 20, patObj);`,
        `            } { |err|`,
        `              ("[energy] override/prefilter failed (" ++ pos ++ "/" ++ pick[\\id] ++ "): " ++ err.errorString).warn;`,
        `              nil`,
        `            };`,
        `            if(patObj.notNil and: { q.notNil }, {`,
        `              cond = Condition(false);`,
        `              q.add([patObj, cond, id]);`,
        `              addr !? { addr.sendMsg("/envilEnergyTok", id, "play", pos.asString, pick[\\id].asString) };`,
        `              maxWait = ((cfg[\\maxDur] ? 20).max(0.1)) + 2;`,
        `              waited = 0;`,
        `              while({ cond.test.not and: { waited < maxWait } }, { 0.1.wait; waited = waited + 0.1 });`,
        `              if(cond.test.not, {`,
        `                ("[energy] token wait timeout at " ++ pos ++ "/" ++ pick[\\id] ++ " (continuing)").warn;`,
        `                cond.test = true;`,
        `              });`,
        ...
        `              }, {`,
        `                addr !? { addr.sendMsg("/envilEnergyTok", id, "skip", pos.asString, pick[\\id].asString) };`,
        `              });`,
        `              dwell.max(0.05).wait;`,
        `            });`,
        `          }, {`,
...
function buildEnergyExtrasCode() {
    return [
        ...
        `  });`,
        `});`,
        `Library.put(\\envil, \\energyPlayNow, { |name, src, patId|`,
        `  var graph = Library.at(\\envil, \\energyGraph);`,
        `  var cfg = Library.at(\\envil, \\energyCfg) ? ();`,
        `  var node = graph !? { graph.at(name.asSymbol) };`,
        `  var sp = Library.at(\\envil, \\energySpawners, name.asSymbol);`,
        `  var pat, po, go, str;`,
        `  if(node.isNil, { ("[energy] play-now skipped (missing node): " ++ name).warn; ^nil });`,
        `  if(sp.isNil, { ("[energy] play-now skipped (missing spawner): " ++ name).warn; ^nil });`,
        `  pat = try { src.interpret } { |err| ("[energy] play-now interpret failed (" ++ name ++ "/" ++ patId ++ "): " ++ err.errorString).warn; nil };`,
        `  if(pat.isKindOf(Pattern).not, {`,
        `    if(pat.notNil, { ("[energy] play-now not a Pattern (" ++ name ++ "/" ++ patId ++ ")").warn });`,
        `    ^nil;`,
        `  });`,
        `  po = Library.at(\\envil, \\energyMkOvr).value(node[\\ovr]);`,
        `  go = Library.at(\\envil, \\energyMkOvr).value(cfg[\\govr]);`,
        `  pat = try {`,
        `    pat = Library.at(\\envil, \\energyApplyOvr).value(pat, po, "pool", name.asString, patId.asString);`,
        `    pat = Library.at(\\envil, \\energyApplyOvr).value(pat, go, "global", name.asString, patId.asString);`,
        `    Pfindur(cfg[\\maxDur] ? 20, pat);`,
        `  } { |err|`,
        `    ("[energy] play-now override/prefilter skipped (" ++ name ++ "/" ++ patId ++ "): " ++ err.errorString).warn;`,
        `    nil`,
        `  };`,
        `  if(pat.isNil, { ^nil });`,
        `  str = sp.par(Pfset({}, Pprotect(pat, { |err| ("[energy] play-now pattern error in " ++ name ++ ": " ++ err.errorString).warn }), {}));`,
        `  ("[energy] play-now " ++ name ++ "/" ++ patId).postln;`,
        `  str;`,
        `});`,
