...
// ── stdout listeners for queryCode ────────────────────────────────────────────
type StdoutListener = (text: string) => void;
const stdoutListeners: StdoutListener[] = [];

// ── sclang exit callbacks ─────────────────────────────────────────────────────
type ExitCallback = (code: number | null) => void;
const exitCallbacks: ExitCallback[] = [];

export function onSclangExit(cb: ExitCallback): void {
    exitCallbacks.push(cb);
}
// ── sclang start callbacks ────────────────────────────────────────────────────
// Fired on EVERY spawn — whether via the startSCLang command OR the implicit
// auto-start when the user evals code (executeBlock/executeCode) with no
// interpreter running. Host-side init (probe, proxy registers, status bar)
// hooks in here so ALL start paths behave identically.
type StartCallback = () => void;
const startCallbacks: StartCallback[] = [];

export function onSclangStart(cb: StartCallback): void {
...
export function addSuppressMarker(marker: string): void {
 ...
 */
function filterMarkers(text: string): string {
    if (suppressMarkers.size === 0) return text;
    _stdoutBuf += text;
...
export async function startSclang(fallbackToExe = true): Promise<boolean> {
    ...
    try {
        ...
        for (const cb of startCallbacks) { try { cb(); } catch (_) { /* host init must not break spawn */ } }
        return true;
    } catch (err) {
 ...
 * with a "-> …" echo line.
 */
export function queryCode(scCode: string, marker: string, timeoutMs = 2000): Promise<string | null> {
    return new Promise((resolve) => {
        if (!sclangProcess || !sclangProcess.stdin) { resolve(null); return; }

        let buf = '';
        let resolved = false;

        const listener: StdoutListener = (text) => {
            buf += text;
            ...
            if (endIdx === -1) return;          // haven't received closing marker yet
            cleanup();
...
export async function checkServerRunning(): Promise<boolean> {
    ...
    const result = await queryCode(
        `"${marker}".post; s.serverRunning.asString.post; "${marker}".postln;`,
        marker,
        2000,
    );
...
export async function probeRunningServer(autoInitProxy = true, inputProxyCode = ''): Promise<boolean> {
    ...
    const rdy = await queryCode(
        `"${rdyMarker}1${rdyMarker}".postln;`,
        rdyMarker,
        15000,   // class library can take a while
    );
    if (rdy?.trim() !== '1') return false;
